<script>
  ...
  function getSelected(t) {
    ...
    else if (peopleIdx != -1) return t.substring(peopleIdx + 19);
    else return "all";
  }
...
<ul class="stats">
  ...
  {#each tags as tag}
    {#if tag.name == selected}
      <li class="my-1">
        <a on:click={tagSelected} class="active" href={tag.href}>{tag.name}</a>
      </li>
    {:else}
      <li class="my-1">
        <a on:click={tagSelected} href={tag.href}>{tag.name}</a>
      </li>
    {/if}
  {/each}
  ...
  {#if selected == 'all' && tags_category == 'circles'}
    <li class="my-1">
      <a on:click={tagSelected} class="active" href="#/circles">ALL</a>
    </li>
  {:else if tags_category == 'circles'}
    <li class="my-1">
      <a on:click={tagSelected} href="#/circles">ALL</a>
    </li>
  {:else if selected == 'all' && tags_category == 'ambassadors'}
    <li class="my-1">
      <a on:click={tagSelected} class="active" href="#/ambassadors">ALL</a>
    </li>
  {:else if tags_category == 'ambassadors'}
    <li class="my-1">
      <a on:click={tagSelected} href="#/ambassadors">ALL</a>
    </li>
  {/if}
</ul>
