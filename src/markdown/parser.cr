class Markdown::Parser
    ...
    def render_header(level, line, increment)
      if next_line_is_all?('=')
        @renderer.begin_header level, @lines[@line+1]
      elsif next_line_is_all?('-')
        @renderer.begin_header level, @lines[@line+1]
      else
          ...
          when '*'
            if pos + 1 < bytesize && str[pos + 1].unsafe_chr == '*'
              if two_stars || has_closing?('*', 2, str, (pos + 2), bytesize)
                ...
                cursor = pos + 1
                if two_stars
                  @renderer.end_bold false
                else
                  @renderer.begin_bold false
                end
                two_stars = !two_stars
              end
              ...
              cursor = pos + 1
              if one_star
                @renderer.end_italic false
              else
                @renderer.begin_italic false
              end
              one_star = !one_star
            end
          when '_'
            if pos + 1 < bytesize && str[pos + 1].unsafe_chr == '_'
              if two_underscores || (last_is_space && has_closing?('_', 2, str, (pos + 2), bytesize))
                ...
                cursor = pos + 1
                if two_underscores
                  @renderer.end_bold true
                else
                  @renderer.begin_bold true
                end
              ...
              cursor = pos + 1
              if one_underscore
                @renderer.end_italic true
              else
                @renderer.begin_italic true
              end
